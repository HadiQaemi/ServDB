...
class CodeToTextPreprocess:
    ...
    def clean_comment_code_line(self, inputLine: str) -> str:
        ...
        filteredInput2 = re.sub("/\*(.*)?", comment_placeholder, inputLine)  # for remove /*comments
        ...
        filteredInput4 = re.sub("\*[\s]?@.*", comment_placeholder,
                                filteredInput3)  # for remove *@test but keep Type Annotations in Java 8:@override |@interface,.
    ...
    def replace_token_param(self, inputLine: str) -> str:
        ...
        """
        inputLine = re.sub(r"/[0-9]{1,50}\/", '/<NUM_PARAM>/', inputLine)  # remove number like  http:test.com/4355/rjr
        inputLine = re.sub(r"=[+-]?[0-9]{1,50}.?[0-9]{,5}&", '=<NUM_PARAM>&',
                           inputLine)  # remove number in url  http:test.com>id=-44.66
        inputLine = re.sub(r"=[+-]?[0-9]{1,50}.?[0-9]{,5}", '=<NUM_PARAM>',
                           inputLine)  # remove number in url  http:test.com>id=44
        return inputLine
    ...
    def preprocess(self, src_dir, domain_file, des_dir, file_type, specify_domain):
        ...
        for filepath in pathlib.Path(train_target_path).glob('**/*.java'):
            ...
            try:
                ...
                target_txt_file_name = r"" + str(des_dir) + str(domain_file) + "_" + file_type
                print(target_txt_file_name)
                ...
                for content in contents:
                    ...
                    if (len(content) > 1):
                        ...
                        if specify_domain == "NO":
                            ...
                            train_line = "" + processed_domain + " " + str(content) # + '<|endoftext|>'
                    wf.write((train_line) + "\n")
                wf.close()
            except:
                print('error')
    ...
    def preprocess_clean_url(self, urlstr_input):
        ...
        try:
            pre_processed = re.sub(r'(https://www.youtube.com/channel/.*<STR_TOKEN>.*)',
                                   "https://www.youtube.com/channel/<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=[A-Za-z0-9-]{20,}&)', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(https://www.youtube.com/playlist?list=[A-Za-z0-9-_]{20,})',
                                   "https://www.youtube.com/playlist?list=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(https://www.youtube.com/watch?v={10,}&)',
                                   "https://www.youtube.com/playlist?list=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(https://www.youtube.com/channel/[A-Za-z0-9-_]{10,})',
                                   "https://www.youtube.com/playlist?list=<STR_TOKEN>", pre_processed)

            pre_processed = re.sub(r'(=[A-Za-z0-9-]{34})', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=[a-z0-9]{27})', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=[a-z0-9]{21})', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=[A-Za-z0-9-_]{39})', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=[A-Za-z0-9]{11,11}&)', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=<STR_TOKEN>[A-Za-z0-9-_]*<STR_TOKEN>*)', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(=<STR_TOKEN>[A-Za-z0-9-_]*)', "=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(playlist?list=[A-Za-z0-9-_]*<STR_TOKEN>[A-Za-z0-9-_]*)',
                                   "playlist?list=<STR_TOKEN>", pre_processed)
            pre_processed = re.sub(r'(https://www.youtube.com/channel/.*<STR_TOKEN>.*)',
                                   "https://www.youtube.com/channel/<STR_TOKEN>", pre_processed)
            # print(urlstr_input)
            # print(pre_processed)
    ...
    def detect_noise(self, text):
        import re
        if re.match(r'^"', text) or ("https://github.com/" in text) or ("https://mail.google.com/" in text) or (
                '"https' in text):
            # if (text.startswith('"')):
            print(text)
            return False;
    ...
    def endpoints(self, src_dir, final_file_name):
        ...
        with open(final_test_txt, 'w') as outfile:
            ...
            for names in all_txt_filenames:
                ...
                with open(path_file) as infile:
                    ...
                    # print("This file is added:" + names)
                    now = datetime.now()
                    current_time = now.strftime("%H:%M:%S")
                    # print("Current Time =", current_time)
                    # print("Duration:" + str(current_time))
                    # print(k)
        ...
        contain_values_http['code'] = contain_values_http['code'].astype(str).apply(self.extracturl_info)
        http_ser_url = contain_values_http[contain_values_http['code'].str.contains(
        ...
        http_ser_url['code'] = http_ser_url['code'].astype(str).apply(self.preprocess_urldecode)
        http_ser_url['code'] = http_ser_url['code'].astype(str).apply(self.preprocess_clean_url)
        http_ser_url['code'] = http_ser_url['code'].astype(str).apply(self.preprocess_clean_url_number_token)
    ...
    def main(self):
        ...
        for train_domain_file in domain_dir:
            self.preprocess(self.src_dir, train_domain_file, self.des_dir, self.file_type, self.specify_domain)