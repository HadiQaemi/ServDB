...
class Collector:
    ...
    def __init__(self, path, config_path, des_direction):
        ...
        self.des_direction = path + des_direction
        self.read_config()
    ...
    def git_header(self):
        return self.config['github_token'][random.randint(0, len(self.config['github_token']) - 1)]
    ...
    def clear_readme(self, status):
        ...
        if status:
            p = subprocess.Popen(
                "pandoc -s " + self.path + "/readme.md",
                stdout=subprocess.PIPE, shell=True)
            # my_string = str(p.communicate())
            soup = BeautifulSoup(str(p.communicate()).replace("', None)", '').replace("(b'", ''), "html.parser")
    ...
    def summarize(self, data):
        ...
        if self.tokenizer == None and self.model == None:
            self.tokenizer = AutoTokenizer.from_pretrained("hadiqaemi/t5-github-readme-summarizer")
            self.model = AutoModelForSeq2SeqLM.from_pretrained("hadiqaemi/t5-github-readme-summarizer")
        inputs = ["summarize: " + str(data)]
        inputs = self.tokenizer(inputs, max_length=max_input_length, truncation=True, return_tensors="pt")
        output = self.model.generate(**inputs, num_beams=20, do_sample=True, min_length=20, max_length=64)
    ...
    def batch_summarize(self, data):
        ...
        if self.tokenizer == None and self.model == None:
            self.tokenizer = AutoTokenizer.from_pretrained("hadiqaemi/t5-github-readme-summarizer")
            self.model = AutoModelForSeq2SeqLM.from_pretrained("hadiqaemi/t5-github-readme-summarizer")
        inputs = self.tokenizer(data, max_length=max_input_length, truncation=True, padding=True, return_tensors="pt")
        output = self.model.generate(**inputs, num_beams=20, do_sample=True, min_length=20, max_length=64)
    ...
    def summarize_items(self, data, desc):
        ...
        #     if self.isNaN(row['readme_cleared']):
        #         inputs.append("summarize: Empty")
        #     else:
        #         inputs.append("summarize: " + row['readme_cleared'])
        # outputs = self.batch_summarize(inputs)
        ...
        print(df)
        # for index, row in df.iterrows():
        #     inputs = ["summarize: " + row['readme_cleared']]
        #     inputs = tokenizer(inputs, max_length=max_input_length, truncation=True, return_tensors="pt")
        #     output = model.generate(**inputs, num_beams=20, do_sample=True, min_length=20, max_length=64)
        #     decoded_output = tokenizer.batch_decode(output, skip_special_tokens=True)[0]
        #     predicted_title = nltk.sent_tokenize(decoded_output.strip())[0]
        #     # df[index]['readme_summarized'] = predicted_title
        #     print(index)
