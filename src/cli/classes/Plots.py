...
class Plots:
    ...
    def plot_wordcloud(self, df: pd.DataFrame, category: str):
        ...
        wordcloud = WordCloud(stopwords=STOPWORDS, background_color="black", max_words=1000).generate(words)
        # print("WordCloud For {}".format(category))
        plt.title("WordCloud For {}".format(category))
        plt.style.use("dark_background")
        plt.imshow(wordcloud, interpolation="bilinear")
        plt.axis("off")
        plt.savefig(self.path + "/images/category/" + category + '.png')
    ...
    def plot_barchart(self, df: pd.DataFrame, clmn: str, title='Bar Chart'):
        ...
        plt.title(title)
    def plot_yearly_statistic(self, df: pd.DataFrame, clmn_target: str, title='sssss Chart'):
        df_all = df
        ...
        plt.grid(False)
        plt.savefig(self.path + "/images/statistic/" + "dataset_statistic_year.png")
    ...
    def chart(self, df):
        ...
        print(extract_cat_arr)

        def extract_featuers_sum3(df: pd.DataFrame, clmn_lable: str, clmn_target_feature: str) -> None:
            ...
            return count_arr
        repository_count_arr = extract_featuers_sum3(df, 'webservice', 'repository_count')
        print(repository_count_arr)
        has_projects_arr = extract_featuers_sum3(df, 'webservice', 'has_projects')
        print(has_projects_arr)
        has_issues_arr = extract_featuers_sum3(df, 'webservice', 'has_issues')
        print(has_issues_arr)
        ...
        print(sex_counts)
        p = ax_year.barh(species, np.array(has_projects_arr), width, label='Projects', color='#b3cccc')
        ax_year.bar_label(p, label_type='center')
        p = ax_year.barh(species, np.array(repository_count_arr), width, left= np.array(has_projects_arr), label='Repository count', color='#686868')
        ax_year.bar_label(p, label_type='center')
        p = ax_year.barh(species, np.array(has_issues_arr), width, label='Issues', left= np.array(repository_count_arr), color='#75a3a3')
        ax_year.bar_label(p, label_type='center')
        ax_year.set_title('Statistic of project and service usage')
        plt.style.use("dark_background")
        ax_year.legend()
        plt.grid(False)
        plt.savefig(self.path + "/images/statistic/" + "dataset_statistic_issues_repo.png")
        return ''